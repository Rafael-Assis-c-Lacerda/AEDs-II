...
class Arvore {
    ...
    public void inserir(Game x) {
        if(raiz == null) raiz = new No(x);
            ...
            if(x.getName().compareTo(raiz.elemento.getName()) > 0) raiz.dir = new No(x);
            ...
            if(x.getName().compareTo(raiz.elemento.getName()) < 0) raiz.esq = new No(x);
    ...
    private void inserir(Game x, No bisavo, No avo, No pai, No no) {
        if(no == null) {
            if(x.getName().compareTo(pai.elemento.getName()) > 0) no = pai.dir = new No(x);
            ...
            if(x.getName().compareTo(no.elemento.getName()) > 0) {
    ...
    private void balancear(No bisavo, No avo, No pai, No no) {
        if(pai.cor == true) {
            if(pai.elemento.getName().compareTo(avo.elemento.getName()) > 0) { 
                if(no.elemento.getName().compareTo(pai.elemento.getName()) > 0) {
...
public class exercicio4{
    ...
    public static void main(String []args) throws FileNotFoundException{
        ...
        try {
            PrintWriter log = new PrintWriter("885033_avinegra.txt"); 
