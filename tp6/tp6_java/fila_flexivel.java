...
class Fila{
    ...
    // Construtor da classe Fila_Flexivel.
    public Fila() {
        primeiro = new Celula();
        ultimo = primeiro;
    }
    ...
    public Game remover() {
        ...
        // Se a fila ficar vazia, atualiza o ponteiro 'ultimo'
    ...
    // Procedimento que mostra os elementos da Fila (formato do main).
...
public class fila_flexivel{
    ...
    public static void main(String []args) throws FileNotFoundException{
        ...
        for(int i = 0; i< n; i++){
            ...
            switch (comando) {
                case "I": // Enfileirar (Inserir no Fim)
                    int fim = scanner.nextInt();
                    for (Celula j = game.primeiro.prox; j != null; j = j.prox) {
                        if (fim == j.elemento.getId()) {
                            pesquisa.inserir(j.elemento);
                            j = game.ultimo;
                        }
                    }
                    break;
                    
                case "R": // Desenfileirar (Remover do Início)
                    Game removidoInicio = pesquisa.remover();
                    System.out.println("(R) " + removidoInicio.getName());
                    break;

                default:
                    // lemos o resto da linha para descartar parâmetros extras
                    scanner.nextLine(); 
                    break;
            }
        }
