...
class Hash {
    ...
    public Hash() {
        tamNormal = 21;
        tamReserva = 9;
        tamTotal = tamNormal + tamReserva;
        tabela = new Game[tamTotal];
        for(int i = 0; i < tamTotal; i++) tabela[i] = null;
        reserva = 0;
    }

    private int h(String x) {
        int soma = 0;
        for(int i = 0; i < x.length(); i++) soma += (int)x.charAt(i);
        return soma % tamNormal;
    }

    public void inserir(Game x) {
        String name = x.getName();
        int pos = h(name);
        if(tabela[pos] == null) tabela[pos] = x;
        else if(reserva < tamReserva){
            for(int i = 0; i < tamReserva; i++) {
                if(tabela[tamNormal + i] == null) {
    ...
    private boolean pesquisarAux(String x) {
        boolean resp = false;
        int pos = h(x);
        ...
        if(tabela[pos].getName().equals(x)) {
            resp = true;
        }
        else if(reserva < tamReserva){
            for(int i = 0; i < tamReserva; i++) {
                if(tabela[tamNormal + i].getName().equals(x)) {
                    resp = true;
                    i = tamReserva;
                }
            }
    ...
    public static void main(String []args) throws FileNotFoundException{
	    ...
	    while (continuar) { //loop para continuar até encontrar o FIM
	        ...
	        if (compare(busca, flag)) { //verifica ocorrencia do FIM e decide se o programa acaba
                ...
                int numeroBusca = Integer.parseInt(busca);
                for(int i = 0; i < jogos; i++) {
                    if(numeroBusca == game[i].getId()) {
                        hash.inserir(game[i]);
                        i = jogos;
                    }
                }
        ...
        try {
            PrintWriter log = new PrintWriter("885033_hashReserva.txt"); 
            log.printf("885033\t%.2fms\n", tempoExecucao); //suponho que o log seja tempo de execução
            log.close();
        } catch (IOException e) {
