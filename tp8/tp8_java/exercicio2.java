...
class Hash {
    ...
    public void inserir(Game x) {
        ...
        int hash = h(name);
        if(tabela[hash] == null) tabela[hash] = x;
        else {
            int rehash = reh(name);
            if(tabela[rehash] == null) tabela[rehash] = x;
        }
    }
    ...
    private boolean pesquisarAux(String x) {
        ...
        if(tabela[hash].getName().equals(x)) {
            resp = true;
        }
        else {
            int rehash = reh(x);
            if(tabela[rehash].getName().equals(x)) {
                resp = true;
            }
        }
        return resp;
...
public class exercicio2{
    ...
    public static void main(String []args) throws FileNotFoundException{
        ...
        try {
            PrintWriter log = new PrintWriter("885033_hashRehash.txt"); 
